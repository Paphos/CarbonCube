...
namespace CarbonCube
{
    ...
    static public class FontSupport
    {
        ...
        /// <param name="msg">message to draw</param>
        /// <param name="drawColor">color to draw in (if null, use black)</param>
        static public void PrintStatus(int index, String msg, Nullable<Color> drawColor)
        {
            LoadFont();
            Color useColor = ColorToUse(drawColor);

            // Compute top left corner as the reference for output status
            Game1.sSpriteBatch.DrawString(sTheFont, msg, (sStatusLocation + new Vector2(0, 25*(index-1))), useColor);
        }

        static public void PrintStatus(int index, String msg)
        {
            PrintStatus(index, msg, null);
        }

        static public void PrintStatusPixelPosition(int pixelX, int pixelY, String msg, Nullable<Color> drawColor)
        {
            LoadFont();
